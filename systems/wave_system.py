"""
wave_system.py - نظام الموجات المتقدم
"""
...
class WaveSystem:
    ...
    def start_wave(self):
        ...
        if self.event_bus:
            self.event_bus.emit('wave_started', {
    ...
    def spawn_enemy(self):
        """توليد عدو"""
        if not self.game_world or not self.game:
            return
    ...
    def spawn_boss(self):
        ...
        self.show_boss_message(boss_type)
        
        stop_background_music()
        start_background_music(self.game.music_muted, True)
        
        if self.event_bus:
            self.event_bus.emit('boss_spawned', {'type': boss_type, 'wave': self.wave_number})
    
    def get_enemy_type(self):
        """اختيار نوع العدو حسب الموجة"""
    ...
    def show_boss_message(self, boss_type):
        """عرض رسالة ظهور البوس"""
        if not self.game:
            return
        ...
        self.game.add_widget(label)
        Clock.schedule_once(lambda dt: self.game.remove_widget(label) if label.parent else None, 3)
    ...
    def end_wave(self):
        ...
        if self.event_bus:
            self.event_bus.emit('wave_completed', {
                'wave': self.wave_number - 1,
                'bonus': bonus,
                'next_wave': self.wave_number
            })
